...
void CreateFlare(short object, int thrown)
{
	...
	if (flare_item != NO_ITEM)
	{
		...
		if (!collision && pos.y <= GetHeight(floor, pos.x, pos.y, pos.z))
		{
			if (!thrown)
			{
				flare->pos.y_rot = lara_item->pos.y_rot - 8192;
...
{
	INJECT(0x00454EE0, set_flare_arm);
	INJECT(0x004549B0, DoFlareInHand);
	INJECT(0x00454750, DoFlareLight);
	INJECT(0x004553B0, draw_flare_meshes);
	INJECT(0x004553E0, undraw_flare_meshes);
	INJECT(0x00455410, ready_flare);
	INJECT(0x00454F50, draw_flare);
	INJECT(0x00454BC0, CreateFlare);
	INJECT(0x00455460, FlareControl);
	INJECT(0x004550C0, undraw_flare);
	INJECT(0x00454A90, DrawFlareInAir);
}
