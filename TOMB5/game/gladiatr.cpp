	...
	if (CreatureActive(item_number))
	{
		...
		else
		{
			...
			case 1:
				...
				if (!(item->ai_bits & 1) && (GetRandomControl() & 0x1F || info.distance <= 1048576 && gladiator->mood == ATTACK_MOOD))
				{
					if (item->ai_bits & 4)
					...
					{
						if (lara.target == item && front && distance < 2359296 && GetRandomControl() & 1 && (lara.gun_type == WEAPON_SHOTGUN || !(GetRandomControl() & 0xF)) && item->mesh_bits == 0xFFFFFFFF)
							item->goal_anim_state = 4;
						else if (info.bite && info.distance < 670761)
			...
			case 2:
				...
				else if (info.distance >= 1048576)
				{
					if (info.bite && info.distance < 4194304)
						item->goal_anim_state = 11;
					else if (!info.ahead || info.distance > 2359296)
						item->goal_anim_state = 3;
				}
				else
					item->goal_anim_state = 1;
			...
			case 3:
				...
				{
					if (gladiator->reached_goal || distance > 4194304)
						item->goal_anim_state = 1;
				}
				else if (gladiator->mood == BORED_MOOD)
					item->goal_anim_state = 2;
